...
Because the number of files necessary to run this chunk is relatively large and inconvenient (separate folder of 12 CSVs), we have set these chunks not to run. Their output can be viewed in the HTML file "Full_Code.html" included in the subfolder "./Full_Code_Run/".
...
There is a separate file for each plot, all downloaded into one folder. Because that's a lot of files/data, we have not included this raw data in our submission, but we can provide it or it can be downloaded at the above links if necessary. All code from section "1.1.2 Reduce environmental data" onward will run using the files provided in the "For_R_Markdown/" folder.
