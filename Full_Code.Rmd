...
# Each plot has 17,520 observations for the year 2021, which is correct:
48*365 # 48 observations per day, 365 days per year
...
unimportant. Rows with more NAs are so rare as to be unimportant as well.
...
# Group.1 and Group.2 are extraneous - they are the same as Day_of_Year and Plot
# Confirm this
...
We'll now do some checks on missing data in this aggregated environmental dataset.
```{r}
...
```
This dataset and all others moving forward are included with the submission. We will comment out all calls to write.csv() so as not to overwrite anything, but include the calls nonetheless so that you can see the file names.
...
(Note: the previous chunk was written by Linnea, the following by Autumn, with slightly different names for the dataframes, hence the inconsistencies.)
...
```{r}
qpcr <- read.csv("./Data/Raw/SPRUCE_qPCR_copy_numbers_2021.csv")
colnames(qpcr)
```
...
```{r}
chloroform <- read.csv("./Data/Raw/SPRUCE_CFE_MBC_MBN_2021_2022.csv")
colnames(chloroform)
```
...
env_data_daily$Sample_date <- as.character(as.Date(env_data_daily$Day_of_Year,origin="2020-12-31"))

# Grab precip data
data <- merge(data,env_data_daily[,c("Sample_date","Plot","PREC_6")],
...
Before we do any proper analyses, let's check the distributions and covariances of our variables. This will tell us if we need to transform any variables, and will also let us check for multicolinearity between variables.
...
Finally, looking at the distributions of the variables (the diagonal of the covariance matrix), it looks like MBN, MBC, bacterial copy number, and archaeal copy number are all left skewed. Let's log transform these.
...
We saw in the covariance matrices that none of the variables have a correlation coefficient above 0.6, so we are not concerned about multicolinearity.
